...
import { connectionString, createDb, closeDb, createRepos } from "@engine/db";
...
const from = arg("from");
const to = arg("to");
const platform = arg("platform") as "meta" | "google" | "tiktok" | undefined;
if (!from || !to) {
  console.error("usage: replay --from <ISO date> --to <ISO date> [--platform meta|google|tiktok]");
  process.exit(1);
}
